...
fn wrap_tool_callback(cb: PyObject) -> Arc<ToolCallback> {
    Arc::new(
        move |func: &CalledFunction, _ctx: &hanzo_engine::ToolCallContext| {
            Python::with_gil(|py| {
...
impl Runner {
    ...
    fn new(
        ...
        let loader = if let Some(amoe_conf) = anymoe_config {
            Box::new(AnyMoeLoader {
                target: loader,
                config: hanzo_engine::AnyMoeConfig {
        ...
        if let Some(mtp_model) = mtp_model {
            pipeline
                .blocking_lock()
                .attach_speculative(SpeculativeConfig::Mtp(hanzo_engine::MtpConfig::new(
                    mtp_model,
                    mtp_n_predict,
                )))
        ...
        if let Some(code_exec) = code_execution_config {
            ...
            #[cfg(not(feature = "code-execution"))]
            {
                ...
                return Err(util::PyApiErr(PyValueError::new_err(
                    "code_execution_config requires the 'code-execution' feature; rebuild hanzo with `--features code-execution`",
                )));
            }
        }
    ...
    fn send_chat_completion_request(
        ...
        Python::with_gil(|py| {
            ...
            let tool_choice = request.tool_choice.as_ref().map(|x| match x {
                ToolChoice::Auto => hanzo_engine::ToolChoice::Auto,
                ToolChoice::NoTools => hanzo_engine::ToolChoice::None,
            });
    ...
    fn import_session(
        ...
        let session: hanzo_engine::SerializedSession = serde_json::from_str(&session_json)
    ...
    fn find_file(&self, file_id: String) -> Option<hanzo_engine::File> {
...
#[pymodule]
fn hanzo(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    initialize_logging();
    ...
    m.add_class::<hanzo_engine::File>()?;
    m.add_class::<hanzo_engine::FileSource>()?;
    m.add_class::<ToolChoice>()?;
    m.add_class::<SpeechGenerationResponse>()?;
    m.add_class::<SpeechLoaderType>()?;

    m.add_class::<hanzo_engine::ResponseMessage>()?;
    m.add_class::<hanzo_engine::Delta>()?;
    m.add_class::<hanzo_engine::ResponseLogprob>()?;
    m.add_class::<hanzo_engine::Logprobs>()?;
    m.add_class::<hanzo_engine::Choice>()?;
    m.add_class::<hanzo_engine::ChunkChoice>()?;
    m.add_class::<hanzo_engine::Usage>()?;
    m.add_class::<hanzo_engine::AgenticToolCallRecord>()?;
    m.add_class::<hanzo_engine::ChatCompletionResponse>()?;
    m.add_class::<hanzo_engine::ChatCompletionChunkResponse>()?;
    m.add_class::<hanzo_engine::CompletionChoice>()?;
    m.add_class::<hanzo_engine::CompletionResponse>()?;
    m.add_class::<hanzo_engine::TopLogprob>()?;
    m.add_class::<hanzo_engine::ModelDType>()?;
    m.add_class::<hanzo_engine::ImageGenerationResponseFormat>()?;
    m.add_class::<McpServerSourcePy>()?;
    m.add_class::<McpServerConfigPy>()?;
    m.add_class::<McpClientConfigPy>()?;
    m.add_class::<hanzo_engine::WebSearchOptions>()?;
    m.add_class::<hanzo_engine::SearchContextSize>()?;
    m.add_class::<hanzo_engine::WebSearchUserLocation>()?;
    m.add_class::<hanzo_engine::ApproximateUserLocation>()?;
    Ok(())
}
