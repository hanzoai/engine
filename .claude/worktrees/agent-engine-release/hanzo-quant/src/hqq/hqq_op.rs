#[cfg(feature = "metal")]
use hanzo_ml::{backend::BackendStorage, DType};
use hanzo_ml::{CpuStorage, CustomOp3, Layout, Result, Shape, WithDType};
...
impl CustomOp3 for Dequant8Bit {
    ...
    fn cpu_fwd(
        ...
        let CpuStorage::U8(w_slice) = w else {
            hanzo_ml::bail!("Weight must be u8, HQQ dequant 8-bit");
        };
        ...
        match (s, z) {
            ...
            (_, _) => hanzo_ml::bail!("Dtype mismatch, expected one of f32, f16, bf16"),
        }
    }
    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        w: &hanzo_ml::MetalStorage,
        l_w: &Layout,
        s: &hanzo_ml::MetalStorage,
        l_s: &Layout,
        z: &hanzo_ml::MetalStorage,
        l_z: &Layout,
...
impl CustomOp3 for Dequant4Bit {
    ...
    fn cpu_fwd(
        ...
        let CpuStorage::U8(w_slice) = w else {
            hanzo_ml::bail!("Weight must be u8, HQQ dequant 4-bit");
        };
        if !(l_w.is_contiguous() && l_s.is_contiguous() && l_z.is_contiguous()) {
            hanzo_ml::bail!("All inputs must be contiguous");
        }
...
impl CustomOp3 for Dequant2Bit {
    ...
    fn cpu_fwd(
        ...
        let CpuStorage::U8(w_slice) = w else {
            hanzo_ml::bail!("Weight must be u8, HQQ dequant 2-bit");
        };
        if !(l_w.is_contiguous() && l_s.is_contiguous() && l_z.is_contiguous()) {
            hanzo_ml::bail!("All inputs must be contiguous");
        }
...
impl CustomOp3 for Dequant1Bit {
    ...
    fn cpu_fwd(
        ...
        let CpuStorage::U8(w_slice) = w else {
            hanzo_ml::bail!("Weight must be u8, HQQ dequant 1-bit");
        };
        if !(l_w.is_contiguous() && l_s.is_contiguous() && l_z.is_contiguous()) {
            hanzo_ml::bail!("All inputs must be contiguous");
        }
...
impl CustomOp3 for Dequant3Bit {
    ...
    fn cpu_fwd(
        ...
        let CpuStorage::I32(w_slice) = w else {
            hanzo_ml::bail!("Weight must be i32, HQQ dequant 3-bit");
        };
        if !(l_w.is_contiguous() && l_s.is_contiguous() && l_z.is_contiguous()) {
            hanzo_ml::bail!("All inputs must be contiguous");
        }
