...
fn validate_mcp_config(config: &McpClientConfig) -> Result<()> {
    ...
    for server in &config.servers {
        ...
        match &server.source {
            hanzo_engine::McpServerSource::Http { url, .. }
            | hanzo_engine::McpServerSource::WebSocket { url, .. } => {
            ...
            hanzo_engine::McpServerSource::Process { command, .. } => {
                if command.is_empty() {
...
async fn main() -> Result<()> {
    ...
    if args.interactive_mode {
        interactive_mode(hanzo, search_embedding_model.is_some(), args.thinking).await;
        return Ok(());
    }
    ...
    let mcp_port = if let Some(port) = args.mcp_port {
        ...
        let mcp_server = mcp_server::create_http_mcp_server(hanzo.clone(), host, port);
