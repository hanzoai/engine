...
impl From<NetworkModePy> for hanzo_engine::NetworkMode {
...
impl From<CodeExecutionPermissionPy> for hanzo_engine::CodeExecutionPermission {
...
impl From<AgentPermissionPy> for hanzo_engine::AgentPermission {
...
impl From<&hanzo_engine::AgentToolSource> for AgentToolSourcePy {
    fn from(value: &hanzo_engine::AgentToolSource) -> Self {
        match value {
            hanzo_engine::AgentToolSource::BuiltIn => Self::BuiltIn,
            hanzo_engine::AgentToolSource::User => Self::User,
            hanzo_engine::AgentToolSource::Mcp => Self::Mcp,
            hanzo_engine::AgentToolSource::External => Self::External,
        }
    }
}
...
impl From<&hanzo_engine::AgentToolKind> for AgentToolKindPy {
    fn from(value: &hanzo_engine::AgentToolKind) -> Self {
        match value {
            hanzo_engine::AgentToolKind::CodeExecution => Self::CodeExecution,
            hanzo_engine::AgentToolKind::WebSearch => Self::WebSearch,
            hanzo_engine::AgentToolKind::File => Self::File,
            hanzo_engine::AgentToolKind::Custom => Self::Custom,
            hanzo_engine::AgentToolKind::External => Self::External,
        }
    }
}
...
/// See the [sandbox reference](/hanzo/reference/sandbox/) for layer
/// details.
#[pyclass]
#[derive(Clone, Debug)]
pub struct SandboxPolicy {
    ...
    pub(crate) network: hanzo_engine::NetworkMode,
...
impl SandboxPolicy {
    ...
    fn new(
        ...
        let network = parse_network_mode(network)?.unwrap_or(hanzo_engine::NetworkMode::Loopback);
        Ok(Self {
            max_memory_mb,
            max_cpu_secs,
            max_procs,
...
impl From<SandboxPolicy> for hanzo_engine::SandboxPolicy {
    fn from(p: SandboxPolicy) -> Self {
        hanzo_engine::SandboxPolicy {
...
pub struct CodeExecutionConfig {
    ...
    pub(crate) permission: Option<hanzo_engine::CodeExecutionPermission>,
...
) -> PyResult<Option<hanzo_engine::CodeExecutionPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(hanzo_engine::CodeExecutionPermission::Auto),
                    PERMISSION_ASK => Ok(hanzo_engine::CodeExecutionPermission::Ask),
                    PERMISSION_DENY => Ok(hanzo_engine::CodeExecutionPermission::Deny),
...
pub(crate) fn parse_agent_permission(
    value: Option<Py<PyAny>>,
) -> PyResult<Option<hanzo_engine::AgentPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(hanzo_engine::AgentPermission::Auto),
                    PERMISSION_ASK => Ok(hanzo_engine::AgentPermission::Ask),
                    PERMISSION_DENY => Ok(hanzo_engine::AgentPermission::Deny),
                    other => Err(PyValueError::new_err(format!(
...
fn parse_network_mode(value: Option<Py<PyAny>>) -> PyResult<Option<hanzo_engine::NetworkMode>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    NETWORK_NONE => Ok(hanzo_engine::NetworkMode::None),
                    NETWORK_LOOPBACK => Ok(hanzo_engine::NetworkMode::Loopback),
                    NETWORK_FULL => Ok(hanzo_engine::NetworkMode::Full),
                    other => Err(PyValueError::new_err(format!(
...
impl From<&hanzo_engine::AgentToolMetadata> for AgentToolMetadataPy {
    fn from(tool: &hanzo_engine::AgentToolMetadata) -> Self {
        Self {
...
impl From<&hanzo_engine::AgentToolApproval> for AgentToolApprovalPy {
    fn from(approval: &hanzo_engine::AgentToolApproval) -> Self {
        Self {
...
impl AgentToolApprovalDecisionPy {
    fn to_core(&self) -> hanzo_engine::AgentToolApprovalDecision {
        match self.decision {
            AgentToolApprovalDecisionKindPy::Approve => {
                hanzo_engine::AgentToolApprovalDecision::approve()
                    .with_remember_for_session(self.remember_for_session)
            }
            AgentToolApprovalDecisionKindPy::Deny => {
                hanzo_engine::AgentToolApprovalDecision::deny(self.message.clone())
            }
        }
    }
}
...
fn parse_agent_approval_result(
    py: Python<'_>,
    result: Py<PyAny>,
) -> PyResult<hanzo_engine::AgentToolApprovalDecision> {
    if let Ok(approve) = result.extract::<bool>(py) {
        return Ok(if approve {
            hanzo_engine::AgentToolApprovalDecision::approve()
        } else {
            hanzo_engine::AgentToolApprovalDecision::deny(None)
        });
    }
...
) -> Option<hanzo_engine::AgentToolApprovalCallback> {
    callback.map(|callback| {
        Arc::new(move |approval: &hanzo_engine::AgentToolApproval| {
            Python::with_gil(|py| {
                let payload = AgentToolApprovalPy::from(approval);
                callback
                    .call1(py, (payload,))
                    .and_then(|result| parse_agent_approval_result(py, result))
                    .unwrap_or_else(|_| hanzo_engine::AgentToolApprovalDecision::deny(None))
            })
        }) as hanzo_engine::AgentToolApprovalCallback
    })
}

impl From<CodeExecutionConfig> for hanzo_engine::CodeExecutionConfig {
    fn from(cfg: CodeExecutionConfig) -> Self {
        let default = hanzo_engine::CodeExecutionConfig::default();
        let approval_callback = cfg.approval_callback.map(|callback| {
            Arc::new(move |approval: &hanzo_engine::CodeExecutionApproval| {
                Python::with_gil(|py| {
            ...
            }) as hanzo_engine::CodeExecutionApprovalCallback
        });
        hanzo_engine::CodeExecutionConfig {
