...
use hanzo_sandbox::{detect, NetworkMode, SandboxPolicy};
use tokio::process::Command;

fn workdir() -> PathBuf {
    let p = std::env::temp_dir().join(format!("hanzo-sandbox-test-{}", std::process::id()));
...
fn seccomp_available(sb: &dyn hanzo_sandbox::Sandbox) -> bool {
    let mut policy = base_policy();
    policy.network = NetworkMode::None;
