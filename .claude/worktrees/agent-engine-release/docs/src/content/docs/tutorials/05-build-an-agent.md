...
Per-request control is on [`RequestBuilder`](https://docs.rs/hanzo/latest/hanzo/struct.RequestBuilder.html): `.with_code_execution()`, `.with_agent_permission(...)`, `.with_agent_approval_callback(...)`, `.with_agent_approval_async_callback(...)`, `.set_max_tool_rounds(...)`, `.with_session_id(...)`, `.with_web_search_options(...)`. Use `stream_chat_request` to observe `Response::AgenticToolCallProgress` events.
