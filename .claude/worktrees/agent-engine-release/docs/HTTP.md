...
- `enable_thinking`: `bool` | `null`. Enable thinking for models that support it. If omitted or `null`, hanzo defers to the chat template's default behavior instead of forcing thinking off. Templates with an explicit `enable_thinking` toggle use the repository fallback of `true` when the flag is omitted.
