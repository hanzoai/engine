...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/v1/upload_image", post(upload_image))
        .route("/v1/upload_text", post(upload_text))
        .route("/v1/upload_audio", post(upload_audio))
        .route("/v1/list_models", get(list_models))
        .route("/v1/select_model", post(select_model))
        .route("/v1/list_chats", get(list_chats))
        .route("/v1/new_chat", post(new_chat))
        .route("/v1/delete_chat", post(delete_chat))
        .route("/v1/load_chat", post(load_chat))
        .route("/v1/rename_chat", post(rename_chat))
        .route("/v1/append_message", post(append_message))
        // Generation settings endpoints
        .route("/v1/settings", get(get_settings))
        // Text-to-speech generation endpoint
        .route("/v1/generate_speech", post(generate_speech))
