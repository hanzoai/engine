...
impl MultiModelBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<Model> {
        ...
        let hanzo = runner_builder.build().await;
...
pub(crate) async fn build_pipeline_from_text_loader(
    ...
    loader: Box<dyn hanzo_engine::Loader>,
) -> anyhow::Result<(Arc<Mutex<dyn Pipeline>>, SchedulerConfig, AddModelConfig)> {
    use hanzo_engine::*;
    ...
    let device_map_setting =
        builder
            .device_mapping
            .clone()
            .unwrap_or(hanzo_engine::DeviceMapSetting::Auto(
                hanzo_engine::AutoDeviceMapParams::default_text(),
            ));
...
pub async fn build_model_from_pipeline(
    pipeline: Arc<Mutex<dyn hanzo_engine::Pipeline>>,
