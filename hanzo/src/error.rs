//! Error types and conversions for the hanzo SDK.

use thiserror::Error;

/// Error type for the hanzo SDK.
///
...
pub enum Error {
    ...
    ModelError {
        ...
        partial_response: Option<Box<hanzo_engine::ChatCompletionResponse>>,
    },
    ...
    /// Wraps the existing [`hanzo_engine::MistralRsError`].
    #[error(transparent)]
    Management(#[from] hanzo_engine::MistralRsError),
...
impl From<tokio::sync::mpsc::error::SendError<hanzo_engine::Request>> for Error {
    /// Convert from a channel send error (mapped to [`Error::Channel`]).
    fn from(e: tokio::sync::mpsc::error::SendError<hanzo_engine::Request>) -> Self {
        Error::Channel(e.to_string())
    }
}

impl From<Box<hanzo_engine::ResponseErr>> for Error {
    /// Convert from a boxed engine response error.
    fn from(e: Box<hanzo_engine::ResponseErr>) -> Self {
        match *e {
            hanzo_engine::ResponseErr::InternalError(inner) => Error::Inference(inner),
            hanzo_engine::ResponseErr::ValidationError(inner) => {
                Error::RequestValidation(inner.to_string())
            }
            hanzo_engine::ResponseErr::ModelError(message, response) => Error::ModelError {
                message,
                partial_response: Some(Box::new(response)),
            },
            hanzo_engine::ResponseErr::CompletionModelError(message, _) => Error::ModelError {
                message,
