...
// Vendored fix (engine branch perf/cuda-flash-attn-prefill): cudarc 0.19.8 split the
// pointer accessors into DevicePtr (device_ptr) and DevicePtrMut (device_ptr_mut). The
// published 0.11.3 imports only DevicePtr, so the four device_ptr_mut call sites fail with
// E0599. Bringing the trait into scope is the ONLY change vs the crates.io 0.11.3 source.
use hanzo_ml::cuda_backend::cudarc::driver::DevicePtrMut;
...
impl FlashAttn {
    ...
    >(
        &self,
        q: &hanzo_ml::CudaStorage,
        q_l: &Layout,
        k: &hanzo_ml::CudaStorage,
        k_l: &Layout,
        ...
        if q_stride[q_rank - 1] != 1 {
            hanzo_ml::bail!("the last dim of q must be contiguous {q_stride:?}")
        }
        if k_stride[k_rank - 1] != 1 {
            hanzo_ml::bail!("the last dim of k must be contiguous {k_stride:?}")
        }
        if v_stride[v_rank - 1] != 1 {
            hanzo_ml::bail!("the last dim of v must be contiguous {v_stride:?}")
        }
        ...
        if expected_kv != v_l.shape().dims4()? {
            hanzo_ml::bail!("shape mismatch q {:?} and v {:?}", q_l.shape(), v_l.shape())
        }
        if head_size_og > 256 {
            hanzo_ml::bail!("only supports head dimension at most 256 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
        ...
        let alibi_slopes_ptr = if let Some(alibi_slopes) = &self.alibi_slopes {
            ...
            let alibi_slopes = match &*alibi_slopes {
                ...
                _ => hanzo_ml::bail!("alibi_slopes must be a cuda tensor"),
            };
        ...
        let dst = hanzo_ml::CudaStorage::wrap_cuda_slice(dst, dev.clone());
        Ok((dst, out_shape))
    }
}

impl hanzo_ml::CustomOp3 for FlashAttn {
    ...
    fn cpu_fwd(
        ...
        hanzo_ml::bail!("no cpu support for flash-attn")
    }
...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let seqlens_q = match &*seqlens_q {
            hanzo_ml::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => hanzo_ml::bail!("seqlens_q must be a cuda tensor"),
        };
        let seqlens_q = match seqlens_q_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_q.slice(o1..o2),
            None => hanzo_ml::bail!("seqlens_q has to be contiguous"),
        };

        let (seqlens_k, seqlens_k_layout) = self.seqlens_k.storage_and_layout();
        let seqlens_k = match &*seqlens_k {
            hanzo_ml::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => hanzo_ml::bail!("seqlens_k must be a cuda tensor"),
        };
        let seqlens_k = match seqlens_k_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_k.slice(o1..o2),
            None => hanzo_ml::bail!("seqlens_k has to be contiguous"),
        };
        ...
        if nseqlens_q < 2 {
            hanzo_ml::bail!("seqlens_q should have a len >= 2 {nseqlens_q}")
        }
        let nseqlens_k = seqlens_k_layout.shape().dims1()?;
        if nseqlens_k != nseqlens_q {
            hanzo_ml::bail!("seqlens_q and seqlens_k should have the same number of elements {nseqlens_q} <> {nseqlens_k}")
        }
...
impl hanzo_ml::CustomOp3 for FlashAttnVarLen {
    fn name(&self) -> &'static str {
        "flash-attn-varlen"
    }
