...
impl From<hanzo_mcp::CodeExecutionPermission> for CodeExecutionPermission {
    fn from(value: hanzo_mcp::CodeExecutionPermission) -> Self {
        match value {
            hanzo_mcp::CodeExecutionPermission::Auto => Self::Auto,
            hanzo_mcp::CodeExecutionPermission::Ask => Self::Ask,
            hanzo_mcp::CodeExecutionPermission::Deny => Self::Deny,
        }
    }
}
...
impl CodeExecutionManager {
    ...
    pub fn get_tool_callbacks(&self, input_modalities: &[InputModality]) -> ToolCallbacksWithTools {
        ...
        let execute_callback: Arc<hanzo_mcp::MultimodalToolCallback> = Arc::new(
            move |func: &CalledFunction, tc: &hanzo_mcp::ToolCallContext| {
        ...
        let reset_callback: Arc<hanzo_mcp::ToolCallback> = Arc::new(
            move |_func: &CalledFunction, tc: &hanzo_mcp::ToolCallContext| {
                let sessions = Arc::clone(&sessions);
                let ctx = ctx.clone();
        ...
        let read_file_stub: Arc<hanzo_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
        ...
        let list_files_stub: Arc<hanzo_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
...
fn denied_by_permission(
    ctx: &SpawnCtx,
    tool_ctx: &hanzo_mcp::ToolCallContext,
    session_id: &str,
    code: &str,
    outputs: &[String],
) -> Option<String> {
    ...
    let reason = match permission {
        ...
        CodeExecutionPermission::Ask => {
            ...
            if let Some(notifier) = &tool_ctx.code_execution_approval_notifier {
                notifier(hanzo_mcp::CodeExecutionApprovalRequest {
                    approval_id,
