...
fn attack_fabricate_hidden_layer_caught_when_opened() {
    ...
    t.matmul(Mat::new(4, 16, rand_i64(64, &mut s)), Mat::new(16, 8, rand_i64(128, &mut s)));
    ...
    t.matmul(Mat::new(4, 8, rand_i64(32, &mut s)), Mat::new(8, 4, rand_i64(32, &mut s)));
    let root = t.root();
    ...
    assert!(!verify_opening(&root, b"beacon", &t.open(1), 2), "the fabricated hidden matmul is caught");
    assert!(verify_opening(&root, b"beacon", &t.open(0), 2), "honest matmul 0 still verifies");
    assert!(verify_opening(&root, b"beacon", &t.open(2), 2), "honest matmul 2 still verifies");
}
...
fn attack_swap_reveal_fails_inclusion() {
    ...
    let op = Opening { index: 0, a, b, c: good, proof: merkle_proof_of(&tr, 0) };
    assert!(
...
fn attack_tamper_input_fails_inclusion() {
    ...
    let op = Opening { index: 0, a: tampered_a, b, c, proof: merkle_proof_of(&tr, 0) };
    assert!(!verify_opening(&tr.root(), b"beacon", &op, 2), "a tampered input breaks the committed leaf");
}
...
fn attack_prefit_challenge_fails_on_fresh_beacon() {
    ...
    assert!(poi::freivalds_verify(&a, &b, &forged, &r1), "forgery survives the GUESSED challenge r1");
...
fn attack_griefer_cannot_frame_honest_prover() {
    ...
    fake.data[0] += 1;
    let op = Opening { index: 0, a, b, c: fake, proof: merkle_proof_of(&tr, 0) };
    ...
    assert!(!verify_opening(&tr.root(), b"beacon", &op, 2), "a non-committed opening cannot slash an honest prover");
}
...
fn defense_two_honest_machines_agree() {
    ...
    assert_eq!(run(), run(), "two honest machines emit the byte-identical transcript root (zero false-reject)");
}
...
fn defense_honest_forward_verifies_under_any_beacon() {
    ...
    let root = t.root();
    for beacon in [b"b0".as_slice(), b"b1", b"another-beacon", b"block:0xdeadbeef"] {
        let idx = challenge_index(beacon, &root, t.len());
        assert!(verify_opening(&root, beacon, &t.open(idx), 2), "honest verifies under beacon {beacon:?}");
    }
}
...
fn attack_quantization_lie_breaks_output_binding() {
    ...
    assert!((lie - dq0).abs() > 0.5, "a lied output diverges from the committed dequant(C)");
}
...
fn defense_shape_mismatch_fails_closed() {
    ...
    assert!(!poi::freivalds_verify(&a, &b, &Mat::new(2, 2, vec![1, 2, 3, 4]), &[1, 1]), "shape mismatch fails closed");
}
