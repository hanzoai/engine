...
#[cfg(test)]
mod tests {
    //! GLM-4.6 / GLM-4.5 / GLM-4.5-Air all carry `architectures[0] =
    //! "Glm4MoeForCausalLM"` and load on this loader unchanged. The real
    //! `zai-org/GLM-4.6` `config.json` is embedded verbatim.
    use super::Glm4MoeConfig;
    use crate::pipeline::NormalLoaderType;

    /// Real `zai-org/GLM-4.6` `config.json`, verbatim.
    const GLM_4_6_CONFIG: &str = r##"{
  "architectures": [
    "Glm4MoeForCausalLM"
  ],
  "attention_bias": true,
  ...
  "vocab_size": 151552
    ...
    #[test]
    fn glm4_6_config_parses() {
        let cfg: Glm4MoeConfig = serde_json::from_str(GLM_4_6_CONFIG)
            .expect("GLM-4.6 config must deserialize into Glm4MoeConfig");
        assert_eq!(cfg.hidden_size, 5120);
        assert_eq!(cfg.num_hidden_layers, 92);
        assert_eq!(cfg.n_routed_experts, 160);
        assert_eq!(cfg.n_shared_experts, 1);
        assert_eq!(cfg.num_experts_per_tok, 8);
        // GLM-4.6 keeps per-head QK-norm and partial rotary.
