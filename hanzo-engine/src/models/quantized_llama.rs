...
impl ModelConfig::FromGGML for ModelWeights {
    fn from_ggml(mut ct: ggml_file::Content, gqa: usize, dtype: DType) -> Result<Self> {
        ...
        let norm = QRmsNorm::new_dtype(ct.remove("norm.weight")?, 1e-5, dtype)?;
        ...
        for layer_idx in NiceProgressBar::<_, 'b'>(
            ...
            layers.push(LayerWeights {
                ...
                attention_norm: QRmsNorm::new_dtype(attention_norm, 1e-5, dtype)?,
                mlp_or_moe,
                ffn_norm: QRmsNorm::new_dtype(ffn_norm, 1e-5, dtype)?,
...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        // PP: each rank loads only its contiguous layer range onto its single device; rank 0 also
        ...
        let (tok_embeddings, norm, output) = if is_head {
            ...
            let tok_embeddings =
                Embedding::new(qtok_embeddings.dequantize(device)?, embedding_length);
            let norm =
            ...
            (Some(tok_embeddings), Some(norm), Some(output))
        } else {
            (None, None, None)
        };
...
impl ModelWeights {
    ...
    pub fn forward(
        ...
        // Run the residual in the model compute dtype (not the F32 the embedding dequantizes to) so
        // the norm/matmul/attention chain stays single-dtype, dropping the F32<->half casts around
        // attention and matching the norm weights (held in self.dtype).
    ...
    fn run_local_layers(&self, h: &Tensor, offsets: &[usize]) -> Result<Tensor> {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            let attn = layer.forward_attn(&x, &dmask, offsets, &positions, &mut cache[i], None)?;
            let x = (attn + residual)?;
            let residual = &x;
            let xn = layer.ffn_norm.forward(&x)?;
            let xn = layer.mlp_or_moe.forward(&xn)?;
            layer_in = (xn + residual)?;
        }
        Ok(layer_in)
    }
}
