...
/// Load an F32 bias tensor and cast to the compute dtype.
fn load_bias<R: std::io::Seek + std::io::Read>(
    ...
    ct.tensor(name, device)?.dequantize(device)?.to_dtype(dtype)
}
...
fn gptoss_swiglu(gate: &Tensor, up: &Tensor, alpha: f32, limit: f32) -> Result<Tensor> {
    ...
    if gate.device().is_cuda() {
        return hanzo_quant::gptoss_swiglu_fused(gate, up, alpha, limit);
    }
    let dtype = gate.dtype();
    let gate_clamped = gate.minimum(
        &Tensor::full(limit, gate.shape(), gate.device())?.to_dtype(dtype)?,
    )?;
    let up_clamped = up.clamp(-(limit as f64), limit as f64)?;
    let gate_scaled = (&gate_clamped * alpha as f64)?;
    let glu = (&gate_clamped * &hanzo_nn::ops::sigmoid(&gate_scaled)?)?;
...
impl GptOssMoe {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let (topk_weight, topk_idx) = {
            ...
            (
                hanzo_nn::ops::softmax_last_dim(&topk.values)?,
                topk.indices,
            )
        };
        ...
        down.broadcast_mul(&topk_weight.to_dtype(out_dtype)?.unsqueeze(D::Minus1)?)?
            .sum(D::Minus2)?
...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        for layer_idx in NiceProgressBar::<_, 'b'>(
            ...
            let wo_b = load_bias(&mut ct, &format!("{prefix}.attn_output.bias"), device, dtype)?;
            ...
            let router_bias = ct
                ...
                .dequantize(device)?
                .to_dtype(DType::F32)?;
            let gate_bias = load_bias(&mut ct, &format!("{prefix}.ffn_gate_exps.bias"), device, dtype)?;
            let up_bias = load_bias(&mut ct, &format!("{prefix}.ffn_up_exps.bias"), device, dtype)?;
            let down_bias = load_bias(&mut ct, &format!("{prefix}.ffn_down_exps.bias"), device, dtype)?;
            let mlp = GptOssMoe {
                router_weight,
                router_bias,
