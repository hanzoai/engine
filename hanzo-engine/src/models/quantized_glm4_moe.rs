...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        for layer_idx in NiceProgressBar::<_, 'b'>(
            ...
            let attention_wq =
                gguf_linear_b(ct.tensor(&format!("{prefix}.attn_q.weight"), device)?, wq_b)?;
            let attention_wk =
                gguf_linear_b(ct.tensor(&format!("{prefix}.attn_k.weight"), device)?, wk_b)?;
            let attention_wv =
                gguf_linear_b(ct.tensor(&format!("{prefix}.attn_v.weight"), device)?, wv_b)?;
