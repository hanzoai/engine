...
impl MLP {
    fn new(cfg: &Config, vb: ShardedVarBuilder, comm: &Arc<hanzo_quant::Comm>) -> Result<Self> {
...
impl Attention {
    ...
    fn forward(
        ...
        let res = self.dense.forward(&attn_output)?;
        Ok(res)
    }
}
...
impl DecoderLayer {
    ...
    fn forward(
        ...
        let attn_outputs = self
            .self_attn
            .forward(&xs, mask, kv_cache, ctx, layer_idx)?;
...
impl Model {
    ...
    pub fn forward(&self, input_ids: &Tensor, ctx: &mut ModelForwardContext<'_>) -> Result<Tensor> {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            xs = layer.forward(&xs, &mask.get(xs.device()), &mut cache[i], ctx, i)?;
        }
