...
impl NormalLoaderType {
    pub fn from_causal_lm_name(name: &str) -> Result<Self> {
        match name {
            ...
            "GlmMoeDsaForCausalLM" => Ok(Self::Glm5Moe),
...
impl AutoNormalLoader {
    fn get_loader(config: &str) -> Result<Box<dyn NormalModelLoader>> {
        ...
        match tp {
            ...
            NormalLoaderType::Glm5Moe => Ok(Box::new(Glm5MoeLoader)),
...
// =============================================================================
// GLM-5 (GlmMoeDsaForCausalLM): DeepSeek-V3.2-style MLA + DSA + MoE with
// interleaved RoPE and IndexShare (cross-layer top-k reuse).
// =============================================================================

/// [`NormalLoader`] for a GLM-5 (`GlmMoeDsaForCausalLM`) model.
pub struct Glm5MoeLoader;

impl NormalModelLoader for Glm5MoeLoader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        Ok(Box::new(models::glm5_moe::Glm5Moe::new(
            &cfg,
            vb,
            self.is_gptx(config)?,
            normal_loading_metadata,
...
impl IsqModelLoader for Glm5MoeLoader {
    fn isq_layer_regexes(&self, config: &str) -> Result<Vec<Regex>> {
        let cfg: crate::models::glm5_moe::Glm5MoeConfig = serde_json::from_str(config)?;
        let mut data = vec![
            Regex::new(r"lm_head\.(weight|bias)$")?,
...
impl DeviceMappedModelLoader for Glm5MoeLoader {
    fn mapped_max_act_size_elems(
        &self,
        config: &str,
        params: &AutoDeviceMapParams,
    ...
    fn layer_sizes_in_bytes(
        ...
        for layer_idx in 0..cfg.num_hidden_layers {
            ...
            let q_proj = {
                let a = cfg.hidden_size * cfg.q_lora_rank;
            ...
            let moe_block = if cfg.is_moe_layer(layer_idx) {
                ...
                let shared = {
                    let inter = cfg.moe_intermediate_size * cfg.n_shared_experts;
                    (h * inter + h * inter + inter * h) / weight_pack_factor
                };
                let gate_weight = cfg.n_routed_experts * cfg.hidden_size + cfg.n_routed_experts;
                gate_proj + up_proj + down_proj + shared + gate_weight
            } else {
                let h = cfg.hidden_size;
...
/// [`NormalLoader`] for a DeepSeek-V4 model (`DeepseekV4ForCausalLM`).
...
impl IsqModelLoader for DeepSeekV4Loader {
    ...
    fn isq_layer_regexes_moqe(&self, config: &str) -> Result<Vec<Regex>> {
        let cfg: crate::models::deepseek4::DeepSeekV4Config = serde_json::from_str(config)?;
        let mut data = vec![Regex::new(r"lm_head\.(weight|bias)$")?];
        for layer_idx in cfg.num_hash_layers..cfg.num_hidden_layers {
            for i in 0..cfg.n_routed_experts {
                data.extend(vec![
                    Regex::new(&format!(
                        r"layers\.{layer_idx}\.mlp\.experts\.{i}\.gate_proj\.(weight|bias)$"
...
impl DeviceMappedModelLoader for DeepSeekV4Loader {
    ...
    fn non_mapped_size_in_bytes(
        ...
        let norm = cfg.hidden_size;
        // Output Hyper-Connection reduction (fn + scale + base).
        let hc_dim = cfg.hc_mult * cfg.hidden_size;
        let output_hc = hc_dim * cfg.hc_mult + 1 + cfg.hc_mult;
        Ok((embed_tokens + lm_head + norm + output_hc) * dtype.size_in_bytes())
    }
    ...
    fn layer_sizes_in_bytes(
        ...
        let hc_mix = 2 * cfg.hc_mult + cfg.hc_mult * cfg.hc_mult;
        let hc_block = hc_mix * (cfg.hc_mult * cfg.hidden_size) + 3 + hc_mix;
        ...
        for layer_idx in 0..cfg.num_hidden_layers {
            ...
            let o_b = cfg.o_lora_rank * cfg.hidden_size / weight_pack_factor;
            let attn = q_a + q_b + kv_a + o_a + o_b;
            ...
            per_layer_elems.push(norms + attn + hc + moe);
        }
...
impl NormalModelLoader for MiniMaxM2Loader {
    fn load(
        ...
        let cfg: crate::models::minimax_m2::MiniMaxM2Config = serde_json::from_str(config)?;
...
impl IsqModelLoader for MiniMaxM2Loader {
    fn isq_layer_regexes(&self, _config: &str) -> Result<Vec<Regex>> {
        Ok(vec![
            ...
            Regex::new(r"layers\.(\d+)\.self_attn\.v_proj\.(weight|bias)$")?,
            Regex::new(r"layers\.(\d+)\.self_attn\.o_proj\.(weight|bias)$")?,
            // MoE experts (per-expert SwiGLU: w1=gate, w3=up, w2=down)
            Regex::new(r"layers\.(\d+)\.block_sparse_moe\.experts\.(\d+)\.w1\.(weight|bias)$")?,
            Regex::new(r"layers\.(\d+)\.block_sparse_moe\.experts\.(\d+)\.w2\.(weight|bias)$")?,
            Regex::new(r"layers\.(\d+)\.block_sparse_moe\.experts\.(\d+)\.w3\.(weight|bias)$")?,
        ])
...
impl DeviceMappedModelLoader for MiniMaxM2Loader {
    ...
    fn layer_sizes_in_bytes(
        ...
        for _ in 0..cfg.num_hidden_layers {
            ...
            // qk-norm is per-layer (over the full flattened q/k projection).
            let q_norm = bias_if!(cfg.use_qk_norm, size_q);
            let k_norm = bias_if!(cfg.use_qk_norm, size_kv);

            // Every layer is a sparse MoE block.
            let gate = cfg.hidden_size * cfg.num_local_experts;
            let routing_bias = bias_if!(cfg.use_routing_bias, cfg.num_local_experts);
            let expert = {
                let h_size = cfg.hidden_size;
                let i_size = cfg.intermediate_size;
                let w1 = h_size * i_size / weight_pack_factor;
                let w3 = h_size * i_size / weight_pack_factor;
                let w2 = i_size * h_size / weight_pack_factor;
                w1 + w2 + w3
            };
            let moe = expert * cfg.num_local_experts + gate + routing_bias;
