...
akin! {
    ...
    impl TryFromValue for *types {
        fn try_from_value(value: gguf_file::Value) -> Result<Self, hanzo_ml::Error> {
            *to_type.or_else(|_| hanzo_ml::bail!("value is not a `*types`"))
        }
    }
}
...
impl<T: TryFromValue> TryFromValue for Vec<T> {
    fn try_from_value(value_vec: gguf_file::Value) -> Result<Self, hanzo_ml::Error> {
        value_vec
            .to_vec()
            .or_else(|_| hanzo_ml::bail!("value is not a `Vec`"))?
            .clone()
            .into_iter()
...
pub trait TryValueInto<T>: Sized {
    fn try_value_into(self) -> Result<T, hanzo_ml::Error>;
}
...
impl<T: TryFromValue> TryValueInto<T> for Option<gguf_file::Value> {
    fn try_value_into(self) -> Result<T, hanzo_ml::Error> {
        match self {
            ...
            None => hanzo_ml::bail!("Expected `Option<gguf_file::Value>` to contain a value"),
        }
    }
}
