...
impl Qwen3AsrPipeline {
    ...
    /// (a Chinese clip decoded as `language English`), derailing the transcript.
    /// Passing `language` seeds the assistant turn with `language <Lang><asr_text>`
    /// so decode is pinned to that language.
    pub fn transcribe_with_language(
        ...
        // are dropped from the returned transcript by the `<asr_text>` split below.
        if let Some(lang) = language {
            let pre = tok
                .encode(format!("language {lang}"), false)
