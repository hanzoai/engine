...
impl ClippableLinear {
    fn new_no_bias(
        ...
        let inner = hanzo_quant::linear_no_bias(in_features, out_features, &None, linear_vb)?;
...
impl Gemma4AudioSSCPConvBlock {
    fn new(
        ...
        assert_eq!(
            kernel_t, kernel_f,
            "Gemma4 SSCP conv2d requires square kernels (hanzo-ml limitation), got ({kernel_t}, {kernel_f})"
        );
        assert_eq!(
            stride_t, stride_f,
            "Gemma4 SSCP conv2d requires square strides (hanzo-ml limitation), got ({stride_t}, {stride_f})"
        );
...
impl Gemma4AudioSubSampleConvProjection {
    fn new(cfg: &Gemma4AudioConfig, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        let input_proj_linear = hanzo_quant::linear_no_bias(
