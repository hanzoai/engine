...
impl EncoderMlp {
    fn new(cfg: &WhisperEncoderArgs, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        let w1 = hanzo_quant::linear_b(dim, hidden_dim, false, &None, vb.pp("w1"))?;
        let w2 = hanzo_quant::linear_b(hidden_dim, dim, use_bias, &None, vb.pp("w2"))?;
        let w3 = hanzo_quant::linear_b(dim, hidden_dim, false, &None, vb.pp("w3"))?;
