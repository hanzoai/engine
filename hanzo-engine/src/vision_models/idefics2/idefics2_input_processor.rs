    ...
    fn preprocess(
        ...
        for image in images.iter_mut() {
            ...
            if config.do_resize.is_some_and(|x| x) {
                ...
                {
                    hanzo_vision::get_resize_image_size(
        ...
        if let Some(max_edge) = self.max_edge {
            images = hanzo_vision::pad_to_max_edge(&images, max_edge);
        }
        ...
        for image in images.iter_mut() {
            ...
            if config.do_pad.is_some_and(|x| x) {
                ...
                let padded = hanzo_vision::pad(&image, max_h as usize, max_w as usize)?;
                let mask = hanzo_vision::make_pixel_mask(&padded, h, w)?;
