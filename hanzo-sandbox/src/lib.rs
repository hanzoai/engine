...
/// A developer toolchain lives behind these: without them a sandboxed shell cannot find
/// python, node, cargo or a conda env, and every command fails on import.
...
/// Preset shape of a policy. `restricted` is the safe floor; `developer` opens what a
/// toolchain needs and is the default once the model can run commands.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
fn apply_developer_profile(policy: &mut SandboxPolicy) {
    ...
    for var in DEVELOPER_ROOT_ENV.iter().chain(DEVELOPER_PATH_ENV) {
