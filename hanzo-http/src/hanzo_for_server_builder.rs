...
pub mod defaults {
    ...
    pub const TOKEN_SOURCE: hanzo_engine::TokenSource = hanzo_engine::TokenSource::CacheToken;
...
impl HanzoForServerBuilder {
    ...
    /// use hanzo_http::hanzo_for_server_builder::HanzoForServerBuilder;
    ///
    /// let builder = hanzo_http::hanzo_for_server_builder::HanzoForServerBuilder::new();
    /// ```
    pub fn new() -> Self {
        Default::default()
    }
    ...
    pub fn with_code_exec_config(mut self, config: hanzo_engine::CodeExecutionConfig) -> Self {
    ...
    pub fn with_code_exec_config_optional(
        mut self,
        config: Option<hanzo_engine::CodeExecutionConfig>,
    ) -> Self {
    ...
    async fn build_single_model(mut self) -> Result<SharedHanzoState> {
        ...
        if let Some(mtp_config) = self.mtp_config.clone() {
            pipeline
                .lock()
                .await
                .attach_speculative(hanzo_engine::SpeculativeConfig::Mtp(mtp_config))?;
        }
    ...
    pub async fn build_multi_model(mut self) -> Result<SharedHanzoState> {
        ...
        for model_config in self.models.iter().skip(1) {
            ...
            let mut add_model_config = hanzo_engine::AddModelConfig::new(engine_config);
            if let Some(mcp_config) = self.mcp_client_config.clone() {
