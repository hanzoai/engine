/**
 * NVFP4 GEMM on Blackwell block-scaled tensor cores (sm_121a).
 *
 * Both operands are E2M1 with an FP8 E4M3 scale per 16 values and one FP32
 * scale for the tensor, so the MMA reads the packed weights directly and no
 * dequantized copy is ever built. Scales ride in the swizzled layout the
 * block-scaled MMA expects; `hanzo_nvfp4_swizzle_*` builds it.
 *
 * Ported from mistral.rs 0a29442f9b.
 */
...
// Upstream asserts 13.3. 13.0 is the oldest toolkit measured to compile this
// and produce correct sm_121a SASS on a GB10, so that is the floor here.
static_assert(CUDART_VERSION >= 13000);

namespace hanzo_nvfp4 {
...
bool valid_dtype(int32_t dtype) {
  return dtype == HANZO_NVFP4_BF16 || dtype == HANZO_NVFP4_F16;
}

bool valid_kernel(int32_t kernel) {
  return kernel == HANZO_NVFP4_PREFILL ||
         kernel == HANZO_NVFP4_DECODE_DP32;
}

bool valid_context(const hanzo_nvfp4_context *context) {
  return context && context->device >= 0 && context->sm_count > 0 &&
         valid_dtype(context->dtype) && valid_kernel(context->kernel);
}

bool valid_shape(const hanzo_nvfp4_shape *shape) {
...
make_arguments(const hanzo_nvfp4_launch &launch) {
...
template <class Kernel>
int prepare_kernel(const hanzo_nvfp4_context &context,
...
template <class Kernel>
int workspace_size(const hanzo_nvfp4_context &context,
                   const hanzo_nvfp4_shape &shape, size_t *bytes) {
  using Adapter = cutlass::gemm::device::GemmUniversalAdapter<Kernel>;
  hanzo_nvfp4_launch launch{};
  launch.shape = shape;
...
template <class Kernel> int gemm(const hanzo_nvfp4_launch &launch) {
  using Adapter = cutlass::gemm::device::GemmUniversalAdapter<Kernel>;
  auto arguments = make_arguments<Kernel>(launch);
  auto status = Adapter::can_implement(arguments);
  if (status != cutlass::Status::kSuccess)
...
using namespace hanzo_nvfp4;

extern "C" const char *hanzo_nvfp4_error_string(int status) {
...
extern "C" int hanzo_nvfp4_prepare(int32_t device, int32_t dtype,
                                       int32_t kernel,
                                       hanzo_nvfp4_resources *resources) {
  ...
  hanzo_nvfp4_context context{device, props.multiProcessorCount, dtype,
                                  kernel};
  if (kernel == HANZO_NVFP4_DECODE_DP32)
    return dtype == HANZO_NVFP4_BF16
...
extern "C" int
hanzo_nvfp4_workspace_size(const hanzo_nvfp4_context *context,
                               const hanzo_nvfp4_shape *shape,
                               size_t *bytes) {
  if (!valid_context(context) || !valid_shape(shape) || !bytes)
    return -static_cast<int>(cudaErrorInvalidValue);
  if (context->kernel == HANZO_NVFP4_DECODE_DP32)
    return context->dtype == HANZO_NVFP4_BF16
...
extern "C" int hanzo_nvfp4_gemm(const hanzo_nvfp4_launch *launch) {
  if (!launch || !valid_context(&launch->context) ||
    ...
    return -static_cast<int>(cudaErrorInvalidValue);
  if (launch->context.kernel == HANZO_NVFP4_DECODE_DP32)
    return launch->context.dtype == HANZO_NVFP4_BF16
...
extern "C" int hanzo_nvfp4_scale_bytes(int32_t rows, int32_t k,
                                           size_t *bytes) {
  size_t cols = 0, padded_cols = 0;
  return scale_sizes(rows, k, &cols, &padded_cols, bytes);
}

extern "C" int hanzo_nvfp4_swizzle_host(const void *source, void *dest,
...
extern "C" int hanzo_nvfp4_swizzle_cuda(const void *source, void *dest,
                                            int32_t rows, int32_t k,
