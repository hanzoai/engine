...
pub use distributed::{
    ...
    pipeline::{broadcast_head_tokens, PpHeader, RingPipeline, PP_OP_FORWARD, PP_OP_TERMINATE},
...
pub fn try_fused_quantized_gate_up(
    ...
    let dtype = gate_q.dtype();
    if dtype != up_q.dtype() || !gguf::fast_mmvq::supports(dtype) {
        return Ok(None);
    }
