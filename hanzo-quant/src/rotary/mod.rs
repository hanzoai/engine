...
mod cuda {
    ...
    where
        T: CudaDType + hanzo_ml::cuda_backend::cudarc::driver::DeviceRepr,
    {
        let RotaryLaunch {
            query,
            query_l,
        ...
        if !cos_cache.device().same_device(&dev) || !sin_cache.device().same_device(&dev) {
            hanzo_ml::bail!("apply-rotary tensors must be on the same cuda device");
        }

        if query_l.stride().len() != 3 {
            hanzo_ml::bail!("apply-rotary expects query rank 3 ({query_l:?})")
        }
        if cos_l.stride().len() != 2 || sin_l.stride().len() != 2 {
            hanzo_ml::bail!("apply-rotary expects rank 2 caches")
        }
        ...
        let positions = if let Some((positions, positions_l)) = positions {
            if positions.dtype() != DType::U32 {
                hanzo_ml::bail!("apply-rotary-positions expects positions to be u32");
            }
            if !positions.device().same_device(&dev) {
                hanzo_ml::bail!("positions must be on the same cuda device as query");
            }
            if positions_l.stride().len() != 1 {
                hanzo_ml::bail!("apply-rotary-positions expects rank 1 positions")
            }
            ...
            let positions = match &positions.slice {
                ...
                _ => hanzo_ml::bail!("positions dtype mismatch"),
            };
            let (positions, guard) =
    ...
    impl InplaceOp3 for RotaryInplace {
        ...
        fn cpu_fwd(
            ...
            hanzo_ml::bail!("apply-rotary-inplace is only supported for cuda")
        }
    ...
    impl InplaceOp3 for RotaryPositionsInplace<'_> {
        ...
        fn cpu_fwd(
            ...
            hanzo_ml::bail!("apply-rotary-positions-inplace is only supported for cuda")
        }
