...
impl QuantizedSerde for GgufMatMul {
        ...
        let mut buffer = if let Some(qw) = qw_opt {
            {
                ...
                // The GGML type id is hanzo-ml's: `to_u32` reads the one
                // `for_each_quant!` table, so every dtype it decodes serializes.
    ...
    fn deserialize(
        ...
        // `from_u32` is the inverse of the id `serialize` writes; both read
        // hanzo-ml's one `for_each_quant!` table.
