//! End-to-end decision-latency benchmark for the rules path: `Heuristic::classify`
//! + `Policy::select` over a realistic ~20-card registry with a ~1k-char prompt.
...
fn local(id: &str, gb: u64, tasks: Vec<Task>, ctx: usize, vision: bool) -> ModelCard {
    ModelCard { id: id.into(), backend: Backend::Local { est_bytes: gb * GB }, tasks, max_context: ctx, vision, cost_per_1k: 0.0 }
}
fn cloud(id: &str, provider: &str, tasks: Vec<Task>, ctx: usize, vision: bool, cost: f64) -> ModelCard {
    ModelCard { id: id.into(), backend: Backend::Cloud { provider: provider.into() }, tasks, max_context: ctx, vision, cost_per_1k: cost }
}
...
fn registry() -> Registry {
    ...
    Registry::new(vec![
        ...
        local("zen-ultra-72b", 80, vec![Reasoning, Math, LongContext, General], 262_144, false),
        local("zen-creative-8b", 9, vec![Creative, General], 32_768, false),
        local("zen-math-7b", 8, vec![Math], 32_768, false),
        cloud("claude-haiku", "anthropic", vec![CheapChat, General], 200_000, false, 0.8),
        cloud("claude-sonnet", "anthropic", vec![Code, Reasoning, Creative, General], 200_000, true, 3.0),
        cloud("claude-opus", "anthropic", vec![Reasoning, Math, LongContext], 200_000, true, 15.0),
        cloud("gpt-5-mini", "openai", vec![CheapChat, General], 128_000, false, 0.6),
        cloud("gpt-5", "openai", vec![Code, Reasoning, General], 256_000, true, 5.0),
        cloud("gpt-5-codex", "openai", vec![Code], 256_000, false, 4.0),
        cloud("gemini-flash", "google", vec![CheapChat, General, LongContext], 1_000_000, true, 0.4),
        cloud("gemini-pro", "google", vec![Reasoning, LongContext, Vision], 2_000_000, true, 4.0),
        cloud("grok-4", "xai", vec![Reasoning, Code, General], 256_000, false, 4.0),
        cloud("deepseek-v3", "deepseek", vec![Code, Math, Reasoning], 128_000, false, 0.3),
    ])
}
...
fn realistic_policy() -> Policy {
    ...
    prefer.insert("code".into(), vec!["zen-coder-24b".into(), "zen-coder-8b".into(), "claude-sonnet".into()]);
    prefer.insert("cheap_chat".into(), vec!["zen-nano-0.6b".into(), "gemini-flash".into()]);
    prefer.insert("reasoning".into(), vec!["zen-reasoner-14b".into(), "zen-ultra-72b".into(), "claude-opus".into()]);
    Policy { prefer, memory_fraction: None, cost_ceiling: Some(10.0) }
}
...
fn bench_rules_path_end_to_end() {
    ...
    let mem = MemSnapshot { available_bytes: 48 * GB, total_bytes: 128 * GB, unified: true };

    let decide = |req: &Request| {
        let task = classifier.classify(black_box(req));
        let ctx = Context {
            task, registry: &reg, mem,
            running: &running, vision_required: req.has_media, min_context: req.approx_tokens,
        };
        policy.select(black_box(&ctx))
    };

    for _ in 0..WARMUP { black_box(decide(&req)); }
    ...
    let t = Instant::now();
    for _ in 0..ITERS { black_box(decide(&req)); }
