...
fn registry() -> Registry {
    use Task::*;
    Registry::new(vec![
        ModelCard { id: "zen-nano-0.6b".into(), backend: Backend::Local { est_bytes: GB }, tasks: vec![CheapChat, General], max_context: 32_768, vision: false, cost_per_1k: 0.0 },
        ModelCard { id: "zen-coder-24b".into(), backend: Backend::Local { est_bytes: 24 * GB }, tasks: vec![Code, Reasoning], max_context: 131_072, vision: false, cost_per_1k: 0.0 },
        ModelCard { id: "zen-agent-4b".into(), backend: Backend::Local { est_bytes: 5 * GB }, tasks: vec![General, Reasoning], max_context: 65_536, vision: false, cost_per_1k: 0.0 },
        ModelCard { id: "claude-sonnet-4-6".into(), backend: Backend::Cloud { provider: "anthropic".into() }, tasks: vec![Code, General], max_context: 200_000, vision: true, cost_per_1k: 0.009 },
        ModelCard { id: "claude-opus-4-8".into(), backend: Backend::Cloud { provider: "anthropic".into() }, tasks: vec![Reasoning, Math], max_context: 200_000, vision: false, cost_per_1k: 0.045 },
        ModelCard { id: "gpt-5.5".into(), backend: Backend::Cloud { provider: "openai".into() }, tasks: vec![Reasoning, Code], max_context: 256_000, vision: false, cost_per_1k: 0.01 },
    ])
}

fn ctx<'a>(task: Task, reg: &'a Registry, avail_gb: u64, running: &'a BTreeSet<String>) -> Context<'a> {
    Context {
        task, registry: reg,
        mem: MemSnapshot { available_bytes: avail_gb * GB, total_bytes: 128 * GB, unified: true },
        running, vision_required: false, min_context: 0,
    }
}
...
fn full_policy_yaml_parses() {
    ...
    assert_eq!(p.prefer["cheap_chat"], vec!["zen-nano-0.6b", "zen5-flash", "claude-haiku-4-5-20251001"]);
}
...
fn zen_local_first_when_ram_available() {
    ...
    assert_eq!(d, Decision::LoadLocal { model: "zen-coder-24b".into(), est_bytes: 24 * GB });
}
...
fn cost_ceiling_blocks_expensive_cloud() {
    ...
    assert_eq!(d, Decision::Cloud { provider: "openai".into(), model: "gpt-5.5".into() });
}
