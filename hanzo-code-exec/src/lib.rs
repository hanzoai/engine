...
use hanzo_sandbox::{NetworkMode, Sandbox, SandboxPolicy};
...
impl From<hanzo_llm_mcp::CodeExecutionPermission> for CodeExecutionPermission {
    fn from(value: hanzo_llm_mcp::CodeExecutionPermission) -> Self {
        match value {
            hanzo_llm_mcp::CodeExecutionPermission::Auto => Self::Auto,
            hanzo_llm_mcp::CodeExecutionPermission::Ask => Self::Ask,
            hanzo_llm_mcp::CodeExecutionPermission::Deny => Self::Deny,
        }
    }
}
...
/// An rlimit shorter than the timeout kills the work before the timeout can report it, so the
/// CPU cap follows the timeout rather than the other way round.
fn raise_cpu_limit_for_timeout(
    ...
    tracing::warn!(
        "code execution timeout is {timeout_secs}s but sandbox max_cpu_secs is {}s; raising max_cpu_secs to {timeout_secs}s",
        policy.max_cpu_secs
    );
...
impl CodeExecutionManager {
    ...
    pub fn network_mode(&self) -> Option<hanzo_sandbox::NetworkMode> {
    ...
    pub fn effective_protection(&self) -> hanzo_sandbox::EffectiveProtection {
    ...
    pub fn get_tool_callbacks(&self, input_modalities: &[InputModality]) -> ToolCallbacksWithTools {
        ...
        let execute_callback: Arc<hanzo_llm_mcp::MultimodalToolCallback> = Arc::new(
            move |func: &CalledFunction, tc: &hanzo_llm_mcp::ToolCallContext| {
        ...
        let reset_callback: Arc<hanzo_llm_mcp::ToolCallback> = Arc::new(
            move |_func: &CalledFunction, tc: &hanzo_llm_mcp::ToolCallContext| {
                let sessions = Arc::clone(&sessions);
                let ctx = ctx.clone();
        ...
        let read_file_stub: Arc<hanzo_llm_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
        ...
        let list_files_stub: Arc<hanzo_llm_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
...
fn denied_by_permission(
    ctx: &SpawnCtx,
    tool_ctx: &hanzo_llm_mcp::ToolCallContext,
    session_id: &str,
    code: &str,
    outputs: &[String],
) -> Option<String> {
    ...
    let reason = match permission {
        ...
        CodeExecutionPermission::Ask => {
            ...
            if let Some(notifier) = &tool_ctx.code_execution_approval_notifier {
                notifier(hanzo_llm_mcp::CodeExecutionApprovalRequest {
                    approval_id,
...
mod tests {
    ...
    impl Sandbox for FakeSandbox {
        ...
        ) -> Result<(), hanzo_sandbox::SandboxError> {
            Ok(())
        }
        ...
        fn effective(&self, _policy: &SandboxPolicy) -> hanzo_sandbox::EffectiveProtection {
            hanzo_sandbox::EffectiveProtection {
    ...
    fn raises_cpu_limit_to_ceiled_timeout_when_rlimits_apply() {
        ...
        raise_cpu_limit_for_timeout(&sandbox, &mut policy, Duration::from_millis(5500));
        assert_eq!(policy.max_cpu_secs, 6);
    }

    #[test]
    fn leaves_cpu_limit_alone_without_rlimits_or_when_already_high_enough() {
        let mut policy = SandboxPolicy {
            max_cpu_secs: 5,
            ..SandboxPolicy::default()
        };
        ...
        assert_eq!(policy.max_cpu_secs, 900);
    }
}
