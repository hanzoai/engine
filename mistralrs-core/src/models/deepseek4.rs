...
// DeepSeek V4 Flash defaults — derived from antirez/ds4 reference engine.
serde_default_fn!(usize, sliding_window_v4, 128);
serde_default_fn!(usize, compress_ratio_v4, 4);
serde_default_fn!(usize, index_n_heads_v4, 64);
serde_default_fn!(usize, index_head_dim_v4, 128);
serde_default_fn!(usize, index_topk_v4, 512);
serde_default_fn!(usize, mtp_num_layers_v4, 0);
...
pub struct DeepSeekV4Config {
    ...
    // ---- V4 Flash additions ----
    /// Raw-row sliding window (ring buffer) length. DeepSeek V4 Flash uses 128 by default.
    #[serde(default = "sliding_window_v4", alias = "attention_sliding_window")]
    pub(crate) sliding_window: usize,
    /// Compressed indexer ratio. K rows beyond the raw window are compressed by this factor.
    #[serde(default = "compress_ratio_v4", alias = "attention_compress_ratio")]
    pub(crate) compress_ratio: usize,
    /// Number of indexer heads (V4 Flash: 64).
    #[serde(default = "index_n_heads_v4", alias = "attention_indexer_head_count")]
    pub(crate) index_n_heads: usize,
    /// Indexer per-head dim (V4 Flash: 128).
    #[serde(default = "index_head_dim_v4", alias = "attention_indexer_key_length")]
    pub(crate) index_head_dim: usize,
    /// Indexer Top-K compressed rows to attend to (V4 Flash: 512).
    #[serde(default = "index_topk_v4", alias = "attention_indexer_top_k")]
    pub(crate) index_topk: usize,
    /// Number of MTP draft heads. Zero disables MTP (default).
    #[serde(default = "mtp_num_layers_v4", alias = "mtp_num_layers")]
...
/// DeepSeek V4 Flash compressed-key indexer.
///
/// In V4 Flash, attention on long contexts is split into two paths:
///   1. A *raw* sliding window of `cfg.sliding_window` recent K/V rows. The
///      MLA attention path operates on this window exactly as in V3.
///   2. A *compressed* indexer that summarises every `cfg.compress_ratio`
///      tokens beyond the window into a single key. The indexer computes
///      per-head query vectors (`indexer.attn_q_b` + `indexer.proj`), scores
///      each compressed row, and selects the Top-K (`cfg.index_topk`) rows
///      whose attended K rows are appended to the raw-window K before the
///      attention dot product.
///
/// The compress projection lives in `kv_a_proj_with_mqa` (V3 already produces
...
struct DeepSeekV4Indexer {
    ...
    /// Per-head-dim layernorm on the indexer Q.
    norm: RmsNorm,
    cfg_index_n_heads: usize,
    cfg_index_head_dim: usize,
    cfg_index_topk: usize,
}
...
impl DeepSeekV4Indexer {
    ...
    fn new(
        ...
        // Only ratio-4 layers carry an indexer. V3 (and V4 ratio-128 / dense
        // early layers) skip it entirely.
        if !layer_has_indexer(cfg, layer_idx) {
            return Ok(None);
        }
        let Some(q_lora_rank) = cfg.q_lora_rank else {
            // V4 indexer requires an active LoRA Q rank to project from.
            return Ok(None);
        };
        let index_q_dim = cfg.index_n_heads * cfg.index_head_dim;
        let attn_q_b = ReplicatedLayer::new(
            q_lora_rank,
            index_q_dim,
            &cfg.quantization_config,
            false,
            mapper.set_device(layer_idx, vb.pp("attn_q_b"), loading_isq),
        )?;
        let proj = ReplicatedLayer::new(
            cfg.hidden_size,
            cfg.index_n_heads,
            &cfg.quantization_config,
            false,
            mapper.set_device(layer_idx, vb.pp("proj"), loading_isq),
        )?;
        ...
        Ok(Some(Self {
            ...
            proj,
            norm,
            cfg_index_n_heads: cfg.index_n_heads,
            cfg_index_head_dim: cfg.index_head_dim,
            cfg_index_topk: cfg.index_topk,
        }))
    }

    /// Score the compressed K rows.
    ///
    /// Inputs:
    ///   - `hidden`: (bs, seq, hidden) — the layer-input residual stream.
    ///   - `q_lora`: (bs, seq, q_lora_rank) — the post-`q_a_layernorm` query
    ///     LoRA projection. Re-using V3's q-LoRA avoids a redundant matmul.
    ///
    /// Returns per-token per-head scores shaped `(bs * seq, index_n_heads)`.
    /// The decode-time TopK selection over compressed cache rows is performed
    /// by the caller against the actual cache; this method computes only the
    /// score vector since the compressed cache is not yet plumbed through
    /// `NormalCache` (see `indexer_select_topk_disabled` below).
    fn score(&self, hidden: &Tensor, q_lora: &Tensor) -> Result<Tensor> {
        let (bs, seq, _) = hidden.dims3()?;
        let q = self.attn_q_b.forward_autocast(q_lora)?;
        let q = q
            .reshape((bs, seq, self.cfg_index_n_heads, self.cfg_index_head_dim))?
            .apply(&self.norm)?;
        let weights = self.proj.forward_autocast(hidden)?; // (bs, seq, n_heads)
        // Reduce per-head Q across the head_dim to scalar magnitudes; the
        // ratio-4 compressed K is contracted by the runtime kernel in the
        // reference, but here we expose a per-head score tensor that the
        // attention path can multiply against compressed rows once the
        // compressed cache is wired in (see the NormalCache extension note
        // below — `indexer_select_topk_disabled` covers the dormant case).
        let q_mag = q.sqr()?.sum(D::Minus1)?.sqrt()?; // (bs, seq, n_heads)
        let scores = weights.broadcast_mul(&q_mag)?;
        Ok(scores.reshape((bs * seq, self.cfg_index_n_heads))?)
    }

    /// No-op placeholder for the compressed-cache Top-K selection.
    ///
    /// The compressed cache extension to `NormalCache` is intentionally
    /// out-of-scope for this pass (see file-level note). When the cache is
    /// wired, this function will be replaced with one that selects the
    /// top-`index_topk` compressed rows from the cache using the per-head
    /// scores, projects their K vectors through `kv_a_layernorm` +
    /// `kv_b_proj`, and returns `(k_extra, v_extra)` for the caller to
    /// concatenate with the raw sliding-window K/V.
    fn indexer_select_topk_disabled(&self) -> usize {
        // Returning the configured top-k makes the disabled path
        // observably correct in metadata-driven code paths.
        self.cfg_index_topk
    }

    fn get_isq_layers(&mut self) -> Vec<&mut Arc<dyn QuantMethod>> {
        vec![&mut self.attn_q_b, &mut self.proj]
    }
}

/// V4 Flash applies the compressed indexer to "ratio-4" layers only. The
/// reference engine schedule is: layers `[0, 1]` are dense (no compression),
/// then every even-indexed layer from 2 onwards is ratio-4 (indexer), and
...
/// DeepSeek V4 Flash Multi-Token Prediction (MTP) draft head.
///
/// In V4 Flash, MTP heads predict `mtp_num_layers` future tokens and are used
/// by the speculative-decoding path when `--mtp` GGUF weights are loaded.
/// MTP weights live in a *sibling* GGUF (per the reference engine layout);
/// loading them through the main `ShardedVarBuilder` is out of scope here.
///
/// This struct preserves the interface so the call site is stable. When MTP
/// weights are absent, `forward` is a no-op passthrough:
/// `mtp_disabled_passthrough` returns the base-model logits unchanged. Once
...
/// Per-head MTP weights. The reference engine names these
/// `mtp.{i}.{hc_head_base, hc_head_fn, hc_head_scale, e_proj, h_proj, enorm, hnorm}`.
/// We carry the shape names here so a future sibling-GGUF loader can populate
/// them without reshuffling the struct.
#[allow(dead_code)]
struct MtpHead {
    hc_head_base: Tensor,
    hc_head_fn: Tensor,
...
impl DeepSeekV4MTP {
    ...
    /// Forward pass.
    ///
    /// If no MTP heads are loaded (the default when weights live in a sibling
    /// GGUF that has not been wired in yet), the base-model logits are
    /// returned unchanged. The named function below makes the no-op
    /// observable at call sites.
    fn forward(&self, base_logits: &Tensor) -> Result<Tensor> {
        if self.heads.is_empty() {
            return self.mtp_disabled_passthrough(base_logits);
        }
        // When MTP weights are present, the draft heads would be combined
        // with the base logits here. The combination math is the speculative
        // path's responsibility; this method returns the base logits because
        // the speculative-decoding pipeline routes draft heads separately.
        // Until the sibling-GGUF loader populates `self.heads`, this branch
        // is unreachable.
        Ok(base_logits.clone())
    }

    /// Named no-op for MTP-disabled mode. Returns the base-model logits.
    fn mtp_disabled_passthrough(&self, base_logits: &Tensor) -> Result<Tensor> {
        Ok(base_logits.clone())
    }
...
impl Attention {
    ...
    fn new(
        ...
        // V4 raw window is a sliding-window ring; non-indexer layers attend
        // only to the window. Plumb the value through `SdpaParams` so the
        // shared `Sdpa` op applies the mask correctly.
        let sliding_window = if cfg.sliding_window > 0 {
    ...
    fn forward(
        ...
        // V4 Flash compressed-K indexer call site.
        //
        // The indexer module (loaded on ratio-4 layers) is available via
        // `self.indexer`. With the compressed cache not yet plumbed through
        // `NormalCache`, the indexer is loaded but idle: we do not invoke
        // `score` here because there is no compressed-row store to gate
        // against. Once the cache extension lands, the call here is:
        //
        //   if let Some(indexer) = &self.indexer {
        //       let scores = indexer.score(xs, &q_lora_intermediate)?;
        //       let (k_extra, v_extra) = compressed_cache.select_topk(
        //           &scores, indexer.cfg_index_topk)?;
        //       // concatenate k_extra/v_extra with raw-window k/v before SDPA.
        //   }
        //
        // For now, the indexer presence is observable via `IsqModel` and the
        // residual tensor map so weights round-trip correctly.
        let _ = self.indexer.as_ref().map(|i| i.indexer_select_topk_disabled());
...
impl DeepSeekV4 {
    pub fn new(
        ...
        let cache = if cfg.sliding_window > 0 {
            EitherCache::Normal(NormalCache::from_types(
                (0..cfg.num_hidden_layers)
                    .map(|_| crate::pipeline::NormalCacheType::SlidingWindow {
                        window: cfg.sliding_window,
                    })
                    .collect(),
            ))
        } else {
            EitherCache::Normal(NormalCache::new(
        ...
        Ok(Self {
            ...
            cfg: ModelConfigMetadata {
                ...
                num_attn_heads: (cfg.num_attention_heads / mapper.get_comm_for(0)?.world_size())
                    .max(1),
                sliding_window: if cfg.sliding_window > 0 {
                    Some(cfg.sliding_window)
                } else {
                    None
                },
    ...
    pub fn forward(
        ...
        // MTP draft heads are routed through the speculative-decoding
        // pipeline when active. Without MTP weights this is a no-op
        // passthrough returning the base logits unchanged.
        self.mtp.forward(&logits)
    }

    /// Number of MTP draft heads exposed by this model. Returns 0 when no
    /// sibling MTP GGUF has been loaded.
    #[allow(dead_code)]
    pub fn mtp_num_heads(&self) -> usize {
        self.mtp.num_heads()
    }
}
...
impl IsqModel for DeepSeekV4 {
    fn get_layers(
        ...
        for (i, layer) in self.layers.iter_mut().enumerate() {
            ...
            if let Some(indexer) = layer.attn.indexer.as_mut() {
                for w in indexer.get_isq_layers() {
                    tensors.push((w, Some(i)));
                }
            }
    ...
    fn residual_tensors(&self) -> Vec<(String, Tensor)> {
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            // V4 indexer norm (ratio-4 layers only).
            if let Some(indexer) = &layer.attn.indexer {
                uvb_l.pp("indexer").pp("norm").add(&indexer.norm);
            }
        }
    ...
    fn residual_tensors_moe_experts_only(&self) -> Option<Vec<(String, Tensor)>> {
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            // V4 indexer weights + norm (ratio-4 layers only).
            if let Some(indexer) = &layer.attn.indexer {
                uvb_l.pp("indexer").pp("attn_q_b").add(&indexer.attn_q_b);
                uvb_l.pp("indexer").pp("proj").add(&indexer.proj);
                uvb_l.pp("indexer").pp("norm").add(&indexer.norm);
            }
        }
