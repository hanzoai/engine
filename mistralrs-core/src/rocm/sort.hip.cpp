...
#define ASORT_OP(T, RUST_NAME, ASC)                                            \
  extern "C" void RUST_NAME(void *x1, void *dst1, const int nrows,             \
    ...
    const hipStream_t custream = (hipStream_t)stream;                        \
    int ncols_pad = next_power_of_2(ncols);                                    \
    T *x_row_padded;                                                           \
    uint32_t *dst_row_padded;                                                  \
    hipMallocAsync((void **)&x_row_padded, ncols_pad * sizeof(T), custream);  \
    hipMallocAsync((void **)&dst_row_padded, ncols_pad * sizeof(uint32_t),    \
                    custream);                                                 \
    ...
    for (int row = 0; row < nrows; row++) {                                    \
      ...
      hipMemcpyAsync(x_row_padded, x_row, ncols * sizeof(T),                  \
                      hipMemcpyDeviceToDevice, custream);                     \
      if (ncols_pad - ncols > 0)                                               \
        hipMemcpyAsync(x_row_padded + ncols, values_padded,                   \
                        (ncols_pad - ncols) * sizeof(T),                       \
                        hipMemcpyHostToDevice, custream);                     \
      hipMemcpyAsync(dst_row_padded, indices_padded,                          \
                      ncols_pad * sizeof(uint32_t), hipMemcpyHostToDevice,    \
                      custream);                                               \
      ...
      if (inplace)                                                             \
        hipMemcpyAsync(x_row, x_row_padded, ncols * sizeof(T),                \
                        hipMemcpyDeviceToDevice, custream);                   \
      hipMemcpyAsync(dst_row, dst_row_padded, ncols * sizeof(uint32_t),       \
                      hipMemcpyDeviceToDevice, custream);                     \
    }                                                                          \
    hipFreeAsync(x_row_padded, custream);                                     \
    hipFreeAsync(dst_row_padded, custream);                                   \
    free(indices_padded);                                                      \
    free(values_padded);                                                       \
  }

ASORT_OP(hip_bfloat16, asort_asc_bf16, true)
ASORT_OP(hip_bfloat16, asort_desc_bf16, false)
...
template <typename T> __device__ __forceinline__ T hanzo_shfl_down(T val, int offset) {
  return __shfl_down_sync(0xffffffffffffffffULL, val, offset);
}
template <> __device__ __forceinline__ __half hanzo_shfl_down<__half>(__half val, int offset) {
  return (__half)__shfl_down_sync(0xffffffffffffffffULL, (float)val, offset);
}
template <> __device__ __forceinline__ hip_bfloat16 hanzo_shfl_down<hip_bfloat16>(hip_bfloat16 val, int offset) {
  return (hip_bfloat16)__shfl_down_sync(0xffffffffffffffffULL, (float)val, offset);
}
...
#pragma unroll
  for (int offset = 16; offset > 0; offset /= 2) {
    T other_val = hanzo_shfl_down(val, offset);
    int other_idx = hanzo_shfl_down(idx, offset);
    if (other_val > val) {
      val = other_val;
      idx = other_idx;
    }
...
extern "C" void topk_bf16(const hip_bfloat16 *input,
                          hip_bfloat16 *values_out, // [nrows, k]
                          uint32_t *indices_out,     // [nrows, k]
                          int nrows, int ncols, int k, int64_t stream) {
  const hipStream_t custream = (hipStream_t)stream;
  ...
  size_t smem_size = ncols * sizeof(hip_bfloat16) + ncols * sizeof(bool);

  topk_kernel<hip_bfloat16><<<nrows, block_size, smem_size, custream>>>(
      input, values_out, indices_out, nrows, ncols, k);
}
...
extern "C" void topk_softmax_bf16(const hip_bfloat16 *input,
                                  hip_bfloat16 *weights_out,
                                  uint32_t *indices_out, int nrows, int ncols,
                                  int k, int64_t stream) {
  const hipStream_t custream = (hipStream_t)stream;
  int block_size = (ncols <= 64)    ? 64
                   ...
                   : (ncols <= 256) ? 256
                                    : 512;
  size_t smem_size = ncols * sizeof(hip_bfloat16) + ncols * sizeof(bool) +
                     k * sizeof(hip_bfloat16) + k * sizeof(int) +
                     k * sizeof(float);
  topk_softmax_kernel<hip_bfloat16>
