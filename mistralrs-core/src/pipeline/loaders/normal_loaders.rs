...
impl FromStr for NormalLoaderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            a => Err(format!("Unknown architecture `{a}`. Possible architectures: `mistral`, `gemma`, `mixtral`, `llama`, `phi2`, `phi3`, `qwen2`, `gemma2`, `starcoder2`, `phi3.5moe`, `deepseekv2`, `deepseekv3`, `deepseekv4`, `qwen3`, `glm4`, `glm4moelite`, `glm4moe`, `qwen3moe`, `smollm3`, `granitemoehybrid`, `gpt_oss`, `qwen3next`.")),
        }
    }
}
...
/// [`NormalLoader`] for a DeepSeek V4 Flash model.
///
/// V4 Flash extends V3 with the compressed indexer, sliding-window raw cache,
/// and (optional) MTP draft heads. The HF state-dict prefix structure stays
/// `model.layers.{i}.self_attn.*` so the V4 loader mirrors V3 for sizing and
/// ISQ predicates; ratio-4 layers additionally carry
/// `model.layers.{i}.self_attn.indexer.{attn_q_b,proj,norm}` weights, which are
/// loaded inside the model when present and silently skipped on ratio-128 /
/// dense layers.
///
/// [`NormalLoader`]: https://docs.rs/mistralrs/latest/mistralrs/struct.NormalLoader.html
...
impl IsqModelLoader for DeepSeekV4Loader {
    fn isq_layer_regexes(&self, config: &str) -> Result<Vec<Regex>> {
        let mut data = vec![
            ...
            Regex::new(r"layers\.(\d+)\.self_attn\.kv_b_proj\.(weight|bias)$")?,
            Regex::new(r"layers\.(\d+)\.self_attn\.o_proj\.(weight|bias)$")?,
            // V4 compressed indexer (ratio-4 layers only — names are matched
            // optimistically; layers without an indexer simply have no weights
            // matching these regexes and the predicate is harmless there).
...
impl DeviceMappedModelLoader for DeepSeekV4Loader {
    ...
    fn layer_sizes_in_bytes(
        ...
        for layer_idx in 0..cfg.num_hidden_layers {
            ...
            // V4 compressed indexer (ratio-4 layers from idx>=2, even-indexed).
            let indexer = if cfg.compress_ratio > 0
                && layer_idx >= 2
                && layer_idx % 2 == 0
                && cfg.q_lora_rank.is_some()
            {
                let q_lora_rank = cfg.q_lora_rank.unwrap();
                let attn_q_b = q_lora_rank * cfg.index_n_heads * cfg.index_head_dim
                    / weight_pack_factor;
                let proj = cfg.hidden_size * cfg.index_n_heads / weight_pack_factor;
                let norm = cfg.index_head_dim;
                attn_q_b + proj + norm
            } else {
                0
            };
