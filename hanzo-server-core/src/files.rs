...
pub async fn get_file(State(state): ExtractedState, Path(id): Path<String>) -> Response {
...
pub async fn list_files(State(state): ExtractedState) -> Response {
    let data: Vec<FileMetadata> = state.list_files().iter().map(|f| metadata(f)).collect();
    Json(serde_json::json!({ "object": "list", "data": data })).into_response()
}

pub async fn delete_file(State(state): ExtractedState, Path(id): Path<String>) -> Response {
...
fn serve_bytes(state: SharedState, id: &str) -> Result<Response, (StatusCode, String)> {
