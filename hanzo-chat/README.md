...
> **Deprecated:** The standalone `hanzo-chat` binary is deprecated. Use `hanzo serve --ui` instead for the same functionality.
>
> **Migration:**
> ```bash
> # Old
> cargo run --release --features cuda --bin hanzo-chat -- --text-model Qwen/Qwen3-4B
>
> # New
...
```bash
cargo run --release --features <specify feature(s) here> --bin hanzo-chat -- \
...
```bash
cargo run --release --features cuda --bin hanzo-chat -- \
