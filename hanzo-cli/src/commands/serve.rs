...
use hanzo_server_core::{
    ...
    route_registry::{RouteInfo, RouteKind, HANZO_API_ROUTES},
};
...
pub(crate) fn log_api_surfaces(host: &str, port: u16) {
    ...
    log_routes(HANZO_API_ROUTES, RouteKind::OpenAi);
    debug!("Available Anthropic-compatible routes:");
    log_routes(HANZO_API_ROUTES, RouteKind::Anthropic);
    debug!("Available additional mistral.rs routes:");
    log_routes(HANZO_API_ROUTES, RouteKind::MistralRs);
}
                ...
                ModelFormat::Gguf | ModelFormat::Ggml => {
                    ...
                    if format.quantized_file.is_none() {
                        ...
                        anyhow::bail!(
                            ...
                            Usage: hanzo run auto -m <hf model/local dir> --format {fmt} -f <filename>\n\n\
...
pub(crate) async fn apply_quant_resolution(
    model_type: &mut ModelType,
    token_source: &hanzo_engine::TokenSource,
    matformer: &MatformerSelection,
) -> Result<()> {
...
pub(crate) fn build_code_exec_config(
    ...
    sandbox_policy: Option<hanzo_sandbox::SandboxPolicy>,
) -> Option<hanzo_engine::CodeExecutionConfig> {
    if !runtime.enable_code_execution {
        return None;
    }
    let mut config = hanzo_engine::CodeExecutionConfig::default();
...
pub(crate) fn extract_sandbox_settings(
    ...
    let mode = match (
        sandbox.mode,
        std::env::var(hanzo_sandbox::SANDBOX_ENV_VAR).ok(),
    ) {
...
pub(crate) fn log_agent_runtime(runtime: &RuntimeOptions, max_tool_rounds: Option<usize>) {
    ...
    let rounds = max_tool_rounds.unwrap_or(hanzo_engine::DEFAULT_MAX_TOOL_ROUNDS);
...
fn search_summary(runtime: &RuntimeOptions) -> String {
    ...
    let model = runtime
        ...
        .map(hanzo_engine::SearchEmbeddingModel::from)
        .unwrap_or_default();
