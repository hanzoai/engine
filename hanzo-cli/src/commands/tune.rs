...
use hanzo_engine::{auto_tune, AutoTuneRequest, FitStatus, ModelSelected, QualityTier};
...
pub async fn run_tune(
    ...
    let requested_isq = requested
        .as_deref()
        .map(|s| {
            hanzo_engine::parse_isq_value(s, None)
    ...
    println!("-------------------");
    if let Some(ref path) = emit_config {
        println!("  hanzo from-config --file {}", path.display());
        println!();
...
fn emit_toml_config(
    ...
    result: &hanzo_engine::AutoTuneResult,
) -> Result<String> {
