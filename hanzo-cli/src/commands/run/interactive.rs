...
//!
//! Ported from hanzo-server/src/interactive_mode.rs
...
async fn oneshot_text(
    ...
    do_code_exec: bool,
    agent_permission: AgentPermission,
    agent_approval_callback: Option<hanzo_engine::AgentToolApprovalCallback>,
    enable_thinking: Option<bool>,
    text: String,
) {
    let sender = hanzo.get_sender(None).unwrap();
    let sampling_params = interactive_sample_parameters(&hanzo);
    ...
    let req = Request::Normal(Box::new(NormalRequest {
        ...
        agent_approval_handler: agent_approval_callback
            .map(hanzo_engine::AgentToolApprovalHandler::from_sync),
...
async fn oneshot_multimodal(
    ...
    let config = hanzo.config(None).unwrap();
...
fn print_stats(
    ...
    if let Some(last_usage) = last_usage {
        ...
        if let Ok(logger) = hanzo.get_logger(None) {
...
async fn text_interactive_mode(
    ...
    let mut sampling_params = interactive_sample_parameters(&hanzo);
...
fn print_agentic_progress(
    tool_name: &str,
    phase: &hanzo_engine::AgenticToolCallPhase,
    files: &[hanzo_engine::files::File],
) {
    use hanzo_engine::{AgenticToolCallData, AgenticToolCallPhase};
...
pub(super) fn agent_approval_callback() -> hanzo_engine::AgentToolApprovalCallback {
    std::sync::Arc::new(move |approval: &hanzo_engine::AgentToolApproval| {
        let _render_guard = AGENTIC_RENDER_LOCK.lock().unwrap();
        ...
        loop {
            ...
            if io::stdin().read_line(&mut input).is_err() {
                return hanzo_engine::AgentToolApprovalDecision::deny(None);
            }
            match input.trim().to_ascii_lowercase().as_str() {
                "y" | "yes" => return hanzo_engine::AgentToolApprovalDecision::approve(),
                "a" | "always" => {
                    return hanzo_engine::AgentToolApprovalDecision::approve_for_session();
                }
                "" | "n" | "no" => {
                    return hanzo_engine::AgentToolApprovalDecision::deny(None);
                }
    ...
    while let Some(resp) = rx.recv().await {
        match resp {
            ...
            Response::AgenticToolCallProgress {
                ...
                let complete = matches!(phase, hanzo_engine::AgenticToolCallPhase::Complete(_));
...
async fn audio_interactive_mode(
    ...
    _agent_approval_callback: Option<hanzo_engine::AgentToolApprovalCallback>,
    _enable_thinking: Option<bool>,
) {
    error!("Audio models are not supported in `hanzo run`. Use `hanzo serve` and the OpenAI-compatible /v1/chat/completions endpoint instead.");
}
