...
use hanzo_server_core::mistralrs_for_server_builder::{defaults, MistralRsForServerBuilder};
...
"#
    ...
    for (prefix, paths) in &groups {
        ...
        output += &format!(
            "|{quant_name}|`hanzo run -m {repo_display}{model_type_flag} --from-uqff {first_file}`|\n"
        );
    }
