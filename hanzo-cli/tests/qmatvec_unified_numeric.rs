...
fn check<T: GgmlType, F: Fn(&mut [u8], usize, usize)>(
    ...
    // means bit-exact (not "within a q8_1 fudge"). dp4a_active() reflects the <T>_FALLBACK env.
...
fn qmatvec_unified_numeric() {
    ...
    // SCALAR-forced (<T>_FALLBACK) vs the EXACT oracle: the ground-truth bit-exact weight-decode
    ...
    std::env::set_var("Q2K_FALLBACK", "1");
    for &(n, k) in shapes {
        check::<BlockQ2K, _>(
            &dev,
            &mut log,
    ...
    std::env::remove_var("Q2K_FALLBACK");
    std::env::set_var("Q3K_FALLBACK", "1");
    for &(n, k) in shapes {
        check::<BlockQ3K, _>(
            &dev,
            &mut log,
    ...
    std::env::remove_var("Q3K_FALLBACK");
...
// the per-type `<T>_FALLBACK` env to force the scalar core, unset to take dp4a, and asserts the
