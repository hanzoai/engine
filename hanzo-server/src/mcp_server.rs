...
use hanzo_engine::SupportedModality;
...
use hanzo_server_core::{
    chat_completion::parse_request, handler_core::create_response_channel, types::SharedState,
};
...
impl McpTool for ChatTool {
    ...
    async fn call(
        ...
        match rx.recv().await {
            Some(hanzo_engine::Response::Done(resp)) => {
                let content = resp
                    .choices
                    .iter()
            ...
            Some(hanzo_engine::Response::ModelError(msg, _)) => {
