...
| `NO_MMAP=1` | Disable memory-mapped file loading, forcing all tensor data into memory |
| `NO_MLA=1` | Disable [MLA](MLA.md) (Multi-head Latent Attention) optimization for DeepSeek V2/V3 and GLM-4.7-Flash |
| `ISQ_SINGLETHREAD=1` | Force ISQ (In-Situ Quantization) to run single-threaded |
| `IGPU_MEMORY_FRACTION` | Memory fraction for integrated/unified-memory CUDA GPUs (e.g. NVIDIA Grace Blackwell, Jetson). Float between 0.0 and 1.0, default: `0.75` |
...
| Variable | Description |
|----------|-------------|
| `METAL_PRECOMPILE=0` | Skip Metal kernel precompilation (useful for CI) |
...
| Variable | Description |
|----------|-------------|
| `MN_GLOBAL_WORLD_SIZE` | Total number of devices across all nodes |
| `MN_HEAD_NUM_WORKERS` | Number of worker nodes |
| `MN_HEAD_PORT` | Port for head node communication |

### Worker Nodes

| Variable | Description |
|----------|-------------|
| `MN_WORKER_SERVER_ADDR` | Address of head server to connect to |
| `MN_WORKER_ID` | This worker's ID |
| `MN_LOCAL_WORLD_SIZE` | Number of GPUs on this node |
| `NO_NCCL=1` | Disable NCCL (use alternative backend) |
