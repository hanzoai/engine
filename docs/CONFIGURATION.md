...
| `HANZO_IGPU_MEMORY_FRACTION` | Memory fraction for integrated/unified-memory CUDA GPUs (e.g. NVIDIA Grace Blackwell, Jetson). Float between 0.0 and 1.0, default: `0.75` |
...
| Variable | Description |
|----------|-------------|
| `HANZO_METAL_PRECOMPILE=0` | Skip Metal kernel precompilation (useful for CI) |
...
| Variable | Description |
|----------|-------------|
| `HANZO_MN_GLOBAL_WORLD_SIZE` | Total number of devices across all nodes |
| `HANZO_MN_HEAD_NUM_WORKERS` | Number of worker nodes |
| `HANZO_MN_HEAD_PORT` | Port for head node communication |

### Worker Nodes

| Variable | Description |
|----------|-------------|
| `HANZO_MN_WORKER_SERVER_ADDR` | Address of head server to connect to |
| `HANZO_MN_WORKER_ID` | This worker's ID |
| `HANZO_MN_LOCAL_WORLD_SIZE` | Number of GPUs on this node |
| `HANZO_NO_NCCL=1` | Disable NCCL (use alternative backend) |
