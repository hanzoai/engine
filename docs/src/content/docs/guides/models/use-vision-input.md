...
If you will send video files, install FFmpeg first. The canonical setup checklist is [Set up video input](/hanzo/guides/models/video-setup/).
...
Video decoding requirements, supported containers, and platform install commands are covered in [Set up video input](/hanzo/guides/models/video-setup/). Per-request frame-sampling controls are not currently exposed.
...
Native formats: `.wav`, `.mp3`, `.flac`, `.ogg`. Other formats use FFmpeg conversion; see [Set up video input](/hanzo/guides/models/video-setup/) for FFmpeg installation.
