...
We provide some chat templates [here](https://github.com/hanzoai/engine/tree/main/chat_templates/), and it is easy to modify or create others to customize chat template behavior.
...
```bash
hanzo serve -p 1234 --log output.log --chat-template ./chat_templates/chatml.json -m meta-llama/Llama-3.2-3B-Instruct
```
...
Some models do not provide a `tokenizer.json` file although Hanzo Engine expects one. To solve this, please run [this](https://github.com/hanzoai/engine/blob/main/scripts/get_tokenizers_json.py) script. It will output the `tokenizer.json` file for your specific model. This may be used by passing the `--tokenizer-json` flag *after* the model architecture. For example:

```bash
$ python3 scripts/get_tokenizers_json.py
Enter model ID: microsoft/Orca-2-13b
$ hanzo serve -p 1234 --log output.log -m microsoft/Orca-2-13b --tokenizer-json tokenizer.json
```
...
3) Run `hanzo serve`, specifying the tokenizer and chat template: `hanzo serve -p 1234 --log output.txt --chat-template chatml.json -m microsoft/Orca-2-13b -t tokenizer.json`
